...
EMOJI_COUNT=$(grep -rE '[\x{1F300}-\x{1F9FF}]|[\x{2600}-\x{26FF}]|[\x{2700}-\x{27BF}]' src/ docs/*.html 2>/dev/null | wc -l)
if [ "$EMOJI_COUNT" -eq 0 ]; then
    echo "   ✅ PASS: No emojis found in HTML files"
else
    echo "   ❌ FAIL: Found $EMOJI_COUNT emojis"
    grep -rE '[\x{1F300}-\x{1F9FF}]|[\x{2600}-\x{26FF}]|[\x{2700}-\x{27BF}]' src/ docs/*.html 2>/dev/null
fi
echo ""
